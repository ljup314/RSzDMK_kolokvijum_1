...
int16_t usartParseInt()
{
	...
	// odstranimo sve karaktere koji nisu cifra ili znak -
		while (((usartPeek() < '0' && usartPeek() != '-') || usartPeek() > '9')) // dodati timeout za domaci
		{
			c = usartGetChar();
		}
		...
		if(usartPeek() == '-')
		{
			c = usartGetChar(); // Ako je prvi karakter stringa minus, ocitavamo taj karakter
			sign = -1; // Karakter je minus pa se daje negativni predznak
		}
